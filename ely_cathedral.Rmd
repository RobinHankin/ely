...
```{r,tedsobs, fig.cap="Ted's observation"}
knitr::include_graphics("googlemap.png")
```

```{r, diagely, fig.cap="Diagrammatic representation of Ely Cathedral (EC), Little Ouse (LO), and Robin Hankin (RH); triangle fattened for clarity.  Distances on a spherical earth",out.width="50%"}
...
this, the maximum likelihood estimate $\hat{\theta}$ for $\theta$ will
be $x$; choosing a student from anywhere on the planet corresponds to
$H_1$.  The likelihood ratio will be given by
...
S[o<x] <- -Inf
plot(o,S+log(x),type="l",lwd=3,log="x",xlab="theta",main="likelihood function for theta [0,1]")
abline(h=c(0,-2))
abline(v=1,col='red')
...
amax <- -1/log(x)
plot(alpha,supp(alpha)-supp(amax),log="",type="l",lwd=3,main="likelihood function for alpha")
abline(h=c(0,-2))
abline(v=amax,lty=2)
abline(v=1,col='red')
...
and $\log(-\log 10^{-20})\simeq 3.82$.

We can plot Support functions (that is, the support for a maximum
likelihood hypothesis agains a null) for $x$ as follows:
