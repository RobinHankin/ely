...
* $\log\left(\frac{-1}{e(1-x)\log(1-x)}\right)= -\log x-1+{\mathcal O}(x)$
* $\log(x^{-1})=-\log(x)$
...
plot(x,x*0 +1,type="n",log="x",ylim=c(0,10),ylab="support agains null")
...
At our value of $x=1.2\times 10^{-4}$ we can calculate the support
against all three nulls:
