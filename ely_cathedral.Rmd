...
Below I reproduce the spherical analysis (in a modified and more
reproducible form), and at the end I follow up Ted's suggestion of
...
With these values, the two terms under the square root in the
...
plot(longx,latx,asp=1/(1-cos(mean(latx*pi/180))),type='b',xlab="longitude",ylab="latitude")
for(i in 1:3){text(LL[i,2],LL[i,1],rownames(LL)[i],pos=4)}

plot(longx,latx,asp=1/(1-cos(mean(latx*pi/180))),type='b',xlab="longitude",ylab="latitude",xlim=c(0.26,0.28),ylim=c(52.39,52.41))
for(i in 1:3){text(LL[i,2],LL[i,1],rownames(LL)[i],pos=4)}
```
...
values are!  And secondly we can calculate the excess angle:
