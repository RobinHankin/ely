...
```{r fig.cap="Diagrammatic representation of Ely Cathedral (EC), Little Ouse (LO), and Robin Hankin (RH); triangle fattened for clarity.  Distances on a spherical earth",out.width="50%"}
...
count, but this is lame at best).
...
There seem to be three different alternative hypotheses one might
entertain:
...
$\operatorname{Prob}\left(x\left|n\right.\right)$ would be $\propto
n(1-x)^{n-1}$, where $x\simeq 1.2\times 10^{-4}$ is the $p$-value
above.  The maximum likelihood estimate for $n$ would be
$n_\mathrm{max}=-\log(1-x)^{-1}\simeq 8334$.  The likelihood ratio
(against a null of $n=1$) would be
$\frac{(1-x)^{1/\log(1-x)-1}}{\log(1-x)}$.  We can plot a support
curve:
...
nmax <- -1/log(1-x)
plot(n,S(n) - S(nmax),xlab="n",log="x",ylab="support",type="l",lwd=3)
abline(v=nmax,lty=2)
abline(h=c(0,-2))
abline(v=1,col='red')
```

We can even create a credible interval based on two units of support:
...
angular separation $\leq\theta$.  That is, he (somehow) identifies a
value of $\theta$, and subsequently chooses a student with angular
separation $\leq\theta$.  A likelihood function for $H_\theta$ might
be

$$
{\mathcal L}\left(H_\theta\right)=\operatorname{Prob}\left(x\left|H_\theta\right.\right)=
\left\{\begin{array}{rr}
\frac{1}{\theta}    & \theta\geqslant x\\
0    & \theta < x
\end{array}\right.$$

(where $x\simeq 1.2\times 10^{-4}$ is the $p$-value above).  With
this, the maximum likelihood estimate for $\theta$ will be $x$;
choosing a student from anywhere on the planet corresponds to $H_\pi$.
likelihood ratio $\frac{{\mathcal L}\left(H_x\right)}{{\mathcal
L}\left(H_\pi\right)}$ will simply be $x^{-1}$.
...
S[o<x] <- -Inf
plot(o,S+log(x),type="l",lwd=3,log="x")
abline(h=c(0,-2))
abline(v=1,col='red')
```
...
$$\operatorname{Prob}(\theta) = \alpha\theta^{\alpha-1},\qquad\theta\in [0,1]$$

Then our null would be $\alpha=1$ (uniform).  The likelihood function
for $\alpha$ is then ${\mathcal L}(\alpha)=\alpha x^{\alpha-1}$ which
gives us $\hat{\alpha}=-1/\log x\simeq 0.11$.  The likelihood ratio
against the null would be
$\hat{\alpha}x^{\hat{\alpha}-1}=-\left[\log(x)\cdot
x^{1+1/\log(x)}\right]^{-1}$.
...
amax <- -1/log(x)
plot(alpha,supp(alpha)-supp(amax),log="",type="l",lwd=3)
abline(h=c(0,-2))
abline(v=amax,lty=2)
abline(v=1,col='red')
```
...
* $-(1-x)^{-1/\log(1-x)-1}/\log(1-x)$ (closest among $n$)
* $x^{-1}$  (restricted choice)
* $-\left[\log(x)\cdot x^{1+1/\log(x)}\right]^{-1}$.  (beta distribution)

We can plot Support functions for observed values $x$ as follows:


```{r}
x <- seq(from=5e-5,to=0.1,len=300)
plot(x,log(-(1-x)^(-1/log(1-x)-1)/log(1-x)),log="x",type="l",ylab="support")
points(x,log(1/x),col='red',type="l")
points(x,log(-1/(log(x)*x^(1+1/log(x)))),col='blue',type="l")
abline(h=0)
abline(v=1.2e-4)
```
