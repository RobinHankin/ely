...
`geosphere`.  We can interpret the smallness of two acute angles `EC_LO_RH` and `LO_RH_EC` in two
ways.  Firstly:
...
not succeeded to date.  Although there is one reasonable null
hypothesis to test, it is not clear what one could infer from its
rejection.  But, more seriously, there is no credible alternative
hypothesis; there is no family of distributions to consider; and no
likelihood approach seems to be appropriate.

And as Ted says, there is certainly an element of selection bias to
consider.  One option would be to condition on the fact that our
houses were roughly aligned alongside Ely Cathedral.  But quantifying
"rough alignment" is not straightforward.  My thinking now is that
"the observation" is actually two non-independent pieces of
information: firstly, Ted's geographical insight about rough
alignment; and secondly the more sophisticated quantitative
mathematical analysis.  With this, a null of "`RH` is at a random
position" is, I would say, reasonable.
...
does).  Using `geosphere`, first with `f=0` for a spherical earth and
then using the default value from WGS-84:
...
Both give a $p$-value of about $1.2\times 10^{-4}$, strong grounds to
reject our null.  But what inference can we draw from this?  One
probability model might be as follows.  Consider $n$ randomly chosen
points on the sphere and our observation comprises the smallest
(absolute) bearing $x$ from `LO-EC`.  The likelihood function for $n$,
that is, $\operatorname{Prob}\left(x\left|n\right.\right)$ would be
$\propto n(1-x)^{n-1}$, where $x\simeq 1.2\times 10^{-4}$ is the
$p$-value above.  The maximum likelihood estimate for $n$ would be
$-\log(1-x)^{-1}\simeq 8334$ and we can plot a support curve:
...
plot(n,supp-max(supp),xlab="n",log="x",ylab="support",type="l",lwd=3)
abline(h=c(0,-2))
```
