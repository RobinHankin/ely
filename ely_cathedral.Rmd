...
(dist_plane <- c(sqrt(sum(LO_t^2)),sqrt(sum(RH_t^2)),sqrt(sum((RH_t-LO_t)^2))))
```
...
angles_plane <- c(
   "EC-LO-RH" = f(dist_plane[c(3,1,2)]),
   "LO-RH-EC" = f(dist_plane[c(2,3,1)]),
   "RH-EC-LO" = f(dist_plane[c(1,2,3)])
)
angles_plane
```
...
We may use Heron's formula $\sqrt{s(a-a)(s-b)(s-c)}$ for the area of
the triangle:

```{r heronsformula}
s <- sum(dist_plane)/2
area_plane <- sqrt(s * prod(s - dist_plane))
area_plane
area_plane/1e6  # square km
```

Above, we see that the area of the triangle is about one square
kilometer.
...
(the above result is given in meters; function `distCosine()` uses
spherical [not ellipsoidal] geometry).  Above we see two things:
...
in very close agreement, differing by $2\times 10^{-7}\,{\mathrm m}$,
...
Now, introduce an ellipsoidal earth.  We will use WGS-84 as
implemented by the `geosphere` package, and assume that the latitude
given by Google Maps is geodetic latitude.

```{r,geospherecomp,echo=TRUE}
angles_g <- 
c(
...
names(angles_g) <- c("RH-LO-EC", "EC-RH-LO","RH-EC-LO")
angles_g
```

We can now compare `angles_g` (ellipsoidal) with `angles` (spherical):

```{r showgcd}
rbind(plane=angles_plane,spherical=angles,ellipsoidal=angles_g)
```

The acute angles are much smaller under spherical or ellipsoidal
geometry.  We see that the acute angles under an ellipsoidal earth are
slightly smaller than under the spherical earth.  We can now give
distances in meters between the three points:

```{r usedistgeo}
jj <- rbind(
dist_plane[c(1,3,2)],
gc_dist*RADIUS_OF_EARTH,
c(`EC-LO` = distGeo(ECg,LOg), `LO-RH` = distGeo(LOg,RHg), `EC-RH` = distGeo(ECg,RHg)))
dimnames(jj) <- list(
    c("plane","spherical","ellipsoidal"),
    c("EC-LO", "LO-RH", "EC-RH"))
jj <- rbind(jj,`sph-ell` = apply(jj,2,function(x){abs(x[3]-x[2])}))
jj
```

So the ellipsoidal distances between the locations differ from the
spherical distances by about 10-30 meters---very small compared with
40km great circle distances.  Further work would include calculating
diagnostics for the acuteness of the triangle using an ellipsoidal
earth.  It would be interesting to see what the area of the triangle
and the closest distance between line RH-EC produced and LO would be
under WGS84 (these were about 0.225 square kilometers and 15m on a
spherical earth).


```{r areapolygon}
area # plane
areaPolygon(rbind(ECg, RHg, LOg)) / 1e6 
```
