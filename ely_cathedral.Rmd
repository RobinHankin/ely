...
```{r showgcd}
rbind(angles_g, angles)
```

We see that the acute angles under an ellipsoidal earth are slightly
smaller than under the spherical earth, but all three are in close
agreement.  We can now give distances in meters between the three
points:
...
gc_dist*RADIUS_OF_EARTH)
dimnames(jj) <- list(shape = c("ellipsoidal","spherical"), c("EC-LO", "LO-RH", "EC-RH"))
jj
```
