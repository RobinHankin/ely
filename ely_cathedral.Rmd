...
Compare with the `geosphere` package:
...
in approximate agreement.  Now calculate the sines and cosines:
...
original observation.  We can interpret the smallness of the angles in
two ways.  Firstly:
...
Above, the elements of `dist` show the distance from RH-EC (produced)
to LO, and LO-EC (produced) to RH, in *meters*.  See how small these
values are!  We can verify against the `geosphere` package:
...
And secondly we can calculate the excess angle:
...
angles_lamb
rbind(angles_lamb,angles_gcd,diff=angles_lamb - angles_gcd)
```

We see that the angles under an ellipsoidal earth are slightly bigger
than under the spherical earth.
...
unlikeliness of the observed close alignment.

To do this, we fix EC and LO, and consider a randomly chosen point P.
We then see if P-EC-EO passes closer to LO than

Observe that this approach contains zero selection bias: we are
not considering any selection that Ted might have carried out.
